...
const reviewCards = [
  { title: 'Daily Review', copy: 'দিনশেষে অসমাপ্ত কাজ, energy এবং আগামীকালের focus লিখুন।' },
  { title: 'Weekly Review', copy: 'Projects, finance, health signals এবং relationship follow-ups একবার scan করুন।' },
  { title: 'Monthly Review', copy: 'Income, expenses, investment notes এবং personal capacity trend compare করুন।' },
];
...
export default function PersonalPage() {
  ...
  return (
    <OsPage
      eyebrow="Personal"
      title="আমি কেমন করছি?"
      subtitle="Health, finance এবং reviews এখন Personal section-এর অধীনে—ব্যক্তিগত capacity ও wellbeing operational work থেকে আলাদা কিন্তু connected।"
      stats={[
        { label: 'Health Logs', value: String(healthEntries.length), detail: 'Sleep, mood, energy, symptoms' },
    ...
    >
      ...
      <section className={styles.section} id="reviews">
        <div className={styles.sectionHeader}><div><h2>Reviews</h2><p>Self-management cadence: daily reset, weekly scan, monthly direction check।</p></div></div>
        <div className={styles.grid}>{reviewCards.map((review) => <article className={styles.card} key={review.title}><p className={styles.cardMeta}>Review cadence</p><h3>{review.title}</h3><p>{review.copy}</p></article>)}</div>
      </section>
    </OsPage>
  );
}
